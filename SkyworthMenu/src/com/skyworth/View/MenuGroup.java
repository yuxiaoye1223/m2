...
public class MenuGroup extends View {
    ...
    private boolean EffectIsOn = false;
    ...
    public MenuGroup(Context context, AttributeSet attrs, int type) {
        ...
        if (Type == MenuGroup.bar_player) {
            BgBtp = SearchDrawable.getBitmap("shortcut_bg_bar_progress");
        } else if (Type == MenuGroup.nobar_player) {
            BgBtp = SearchDrawable.getBitmap("shortcut_bg_bar_info");
        } else
            BgBtp = SearchDrawable.getBitmap("shortcut_bg_bar");
        leftBtp = SearchDrawable.getBitmap("shortcut_bg_arrow_left");
        rightBtp = SearchDrawable.getBitmap("shortcut_bg_arrow_right");
    ...
    public void UpdataStatus(String ID) {
        ...
        if (Type == MenuGroup.bar_player)
            this.postInvalidate(0, 90 + BeatHight, this.getWidth(),
                this.getHeight());
        else if (Type == MenuGroup.nobar_player)
            this.postInvalidate(0, 75 + BeatHight, this.getWidth(),
                this.getHeight());
        else
            this.postInvalidate();
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        Log.d(TAG, " MenuGroup_onKey  start " + AbsoluteFoucsID + "   "
            + RelativeFoucsID);
        switch (keyCode) {
            case 0xCF:
            case 0xCE:
            ...
            case KeyEvent.KEYCODE_DPAD_LEFT:
                if (AbsoluteFoucsID > 0) {
                    ...
                    MyCheckedMenu.CheckedMenuHandle(KeyEvent.KEYCODE_DPAD_LEFT,
                        RelativeFoucsID, getAbsoluteFoucsItem());
                }
                Log.d(TAG, " MenuGroup_onKey  finish " + AbsoluteFoucsID
                    + "   " + RelativeFoucsID);
                return true;
            case KeyEvent.KEYCODE_DPAD_RIGHT:
                if (AbsoluteFoucsID + 1 < ItemTotal) {
                    AbsoluteFoucsID++;
                    RelativeFoucsID = AbsoluteFoucsID
                        % GlobalConst.MENU_COUNT_PERPAGE_SHOW;
                    if (RelativeFoucsID == 0)
    ...
    public void myDraw(Canvas canvas, int hight) {
        ...
        this.leftoffset = 0;
        if (CurShowCount < GlobalConst.MENU_COUNT_PERPAGE_SHOW) {
            this.leftoffset = (GlobalConst.MENU_COUNT_PERPAGE_SHOW - CurShowCount)
                / 2
                * GlobalConst.MENU_ITEM_WIDTH
                + (GlobalConst.MENU_COUNT_PERPAGE_SHOW - CurShowCount)
                % 2
                * GlobalConst.MENU_ITEM_WIDTH / 2;
        }

        if (BgBtp != null)
            canvas.drawBitmap(BgBtp, this.getPaddingLeft(),
                this.getPaddingTop() + hight, null);
        ...
        for (int i = 0; i < CurShowCount; i++) {
            if (i == RelativeFoucsID) {
                if (FocusBitmapSet.get(StartIndex + i) == null)
                    FocusBitmapSet.set(StartIndex + i,
                    		SearchDrawable.getBitmap(Menu.get(StartIndex + i) + "sel"));
                bitmap = FocusBitmapSet.get(StartIndex + i);
            } else {
                if (unFocusBitmapSet.get(StartIndex + i) == null)
                    unFocusBitmapSet.set(StartIndex + i,
                    		SearchDrawable.getBitmap(Menu.get(StartIndex + i) + "unsel"));
                bitmap = unFocusBitmapSet.get(StartIndex + i);
            }
            if (Type == MenuGroup.bar_player)
                canvas.drawBitmap(bitmap, this.getLeft()
                    + GlobalConst.MENU_ITEM_WIDTH * i + 35 + leftoffset,
                    0 + 90 + hight, null);
            else if (Type == MenuGroup.nobar_player)
                canvas.drawBitmap(bitmap, this.getLeft()
                    + GlobalConst.MENU_ITEM_WIDTH * i + 35 + leftoffset,
        ...
        if (CurShowPage > 0) {
            if (Type == MenuGroup.nobar_player || Type == MenuGroup.bar_player)
                canvas.drawBitmap(leftBtp, this.getLeft() + 8, 140 + hight,
                    null);
            else
                canvas
                    .drawBitmap(leftBtp, this.getLeft() + 8, 60 + hight, null);
        }

        if ((ItemTotal - StartIndex) > CurShowCount) {
            if (Type == MenuGroup.nobar_player || Type == MenuGroup.bar_player)
                canvas.drawBitmap(rightBtp, this.getLeft() + 1885, 140 + hight,
                    null);
            else
                canvas.drawBitmap(rightBtp, this.getLeft() + 1885, 60 + hight,
                    null);
        }
    }
    ...
    private void UpdataProgress(Canvas canvas, int hight) {
        if (Type != MenuGroup.noplayer) {
            if (bartotal != 0 && bar_cur_pos != 0) {
                ...
                if (dst_width < 1)
                    dst_width = 1;
                Log.d(TAG, " onDraw progressbar cur_pos: " + bar_cur_pos
                    + " tota: " + bartotal + " dst_width : " + dst_width);
                canvas.drawRect(44, 70 + hight, 44 + dst_width, 90 + hight,
                    Rectpaint);
                canvas.drawText(formatSeconds2HHmmss(bar_cur_pos) + '/'
                    + formatSeconds2HHmmss(bartotal), 1600, 45 + hight,
                    Fontpaint);
                // Log.d(TAG," OnDraw : play time:" +
                // formatSeconds2HHmmss(bar_cur_pos)+'/'
    ...
    public void refreshUI(int keyCode, int RelativeFoucsID, int AbsoluteFoucsID) {
        if (Type == MenuGroup.bar_player)
            this.invalidate(0, 90 + BeatHight, this.getWidth(),
                this.getHeight());
        else if (Type == MenuGroup.nobar_player)
            this.invalidate(0, 75 + BeatHight, this.getWidth(),
                this.getHeight());
        else
            this.invalidate();
    ...
    Bitmap myMainBitmap = Bitmap.createBitmap(1920, 238,
        Bitmap.Config.ARGB_8888);
    private boolean first = true;
    // Bitmap myMainBitmap = null;
                ...
                while (!isInterrupted()) {
                    try {
                        if (Myeffect) {
                            ...
                            MenuGroup.this.postInvalidate(0, 0, 1920,
                                MenuGroup.this.getHeight());
                            if (myfisrt == true)
                            ...
                            if (path >= 0 && myfisrt == false) {
                                ...
                                if (path >= 100) {
                                    ...
                                    msg.what = 1;
                                    handlerShowSelectFrame.sendMessage(msg);
                                    // handlerShowSelectFrame.sendMessageDelayed(msg,500);
                                    break;
                                }
                            }
                        }
    ...
    private Handler handlerShowSelectFrame = new Handler() {
    ...
    public void setMGFirstOn() {
        first = true;
    }

    public void setEffectIsOn(boolean isRun) {
        EffectIsOn = isRun;
    }

    public boolean getEffectIsOn() {
        return EffectIsOn;
    }
    ...
    public void HandleSpecialKeyID(int key) {
        ...
        if (focus != -1) {
            ...
            this.onKeyDown(KeyEvent.KEYCODE_ENTER, new KeyEvent(
